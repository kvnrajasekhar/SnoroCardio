...
import { db } from "../lib/firebase"; // Assuming firebase configuration is exported here
...
function HomeScreen() {
    ...
    const fetchGuardians = async () => {
      try {
        ...
        if (userDocSnap.exists()) {
          ...
          const guardianRefs = userData.guardians; // Array of references

          if (Array.isArray(guardianRefs) && guardianRefs.length > 0) {
            // Fetch guardian documents
            const guardianPromises = guardianRefs.map((ref) => getDoc(ref));
            const guardianSnapshots = await Promise.all(guardianPromises);

            const guardiansData = guardianSnapshots.map((snapshot) => ({
              id: snapshot.id,
              ...snapshot.data(),
            }));
  ...
  const handleGuardianChange = async (index, field, value) => {
    const updatedGuardians = [...guardians];
    const guardianToUpdate = updatedGuardians[index];
    

    guardianToUpdate[field] = value;
    setGuardians(updatedGuardians);
  
    try {
      // Update the Firestore document
      const guardianDocRef = doc(db, "guardians", guardians[index].id);
      ...
      alert("Failed to update guardian in the database.");
    }
  };
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gradient-to-b from-[#48cae4] to-[#ade8f4] text-gray-900">
      ...
      <footer className="p-6">
        ...
        >
          Logout
          <FaSignOutAlt className="ml-2 inline" />
        </button>
      </footer>
    </div>
  );
